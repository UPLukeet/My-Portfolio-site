...
function Game() {
    ...
    const [cardQueue, setCardQueue] = useState([Data[0], Data[1], Data[0]]);
        ...
        <div className='cardStyles'>
            <div className={cardTransition}>
                    ...
                    {cardQueue.map((Projects, index) => {
                        return <TinderCard  key={Projects.workName + index } onCardLeftScreen={CardLeftScreen} className='Cards'>
                            <Carousel showThumbs={false} infiniteLoop={true} swipeable={false} emulateTouch={false} showStatus={false} autoPlay={slideShow} dynamicHeight={false}>
                                {Projects.Images && Projects.Images.map((Image, index) => { return <div  key={Image + index} className='image-iframeContainer'><img alt='Images of games' src={require("../assets/Card-images/Games/" + Image)} /></div> })}
